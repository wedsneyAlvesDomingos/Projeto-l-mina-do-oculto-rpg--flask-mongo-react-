...
import { especiesMap, determinarTamanho } from '../../../data/constants/index';
    ...
    return (
        <Paper elevation={4} sx={{ ...sectionStyle, mb: 3, overflow: 'hidden' }}>
            <Box sx={cardHeaderStyle}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 2 }}>
                    ...
                    {!editMode ? (
                        ...
                        <Box>
                            <IconButton onClick={saveCharacter} sx={{ color: '#454E30' }}>
                                <SaveIcon />
                            </IconButton>
                            <IconButton onClick={cancelEdit} sx={{ color: '#931C4A' }}>
                                <CancelIcon />
                            </IconButton>
                        </Box>
                    )}
            ...
            <Box sx={{ p: 3 }}>
                    ...
                    <Grid item xs={12} md={9}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} md={4}>
                                <MoedasBox character={character} atualizarMoeda={atualizarMoeda} />
                            </Grid>
                ...
                {/* Accordions de Antecedente e Descrição */}
                <Box sx={{ mt: 2 }}>
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={6}>
                            <AntecedenteAccordion character={character} editMode={editMode} updateField={updateField} />
                        </Grid>
                        <Grid item xs={12} md={6}>
                            <DescricaoAccordion character={character} editMode={editMode} updateField={updateField} />
                        </Grid>
                    </Grid>
                </Box>
            </Box>
...
// Sub-componente de Moedas
const MoedasBox = ({ character, atualizarMoeda }) => (
    <Box sx={{ backgroundColor: '#f5ede4', borderRadius: 2, p: 1.5, border: '1px solid #BB8130' }}>
        ...
        <Grid container spacing={1}>
            {[
                { tipo: 'platina', label: 'Platina', bg: '#e0e5e0', color: '#756A34' },
                { tipo: 'ouro', label: 'Ouro', bg: '#f5ebe3', color: '#BB8130' },
                { tipo: 'prata', label: 'Prata', bg: '#e8e8e8', color: '#666' },
                { tipo: 'cobre', label: 'Cobre', bg: '#f0dcc8', color: '#8B4513' },
            ].map(({ tipo, label, bg, color }) => (
                <Grid item xs={6} sm={3} key={tipo}>
                    <Box sx={{ textAlign: 'center', p: 0.5, backgroundColor: bg, borderRadius: 1 }}>
                        <Typography variant="caption" sx={{ color: '#5B1F0F', display: 'block', fontSize: '10px' }}>{label}</Typography>
                        <TextField
                            type="number"
                            size="small"
                            variant="standard"
                            value={character.moedas?.[tipo] || 0}
                            onChange={(e) => atualizarMoeda(tipo, e.target.value)}
                            InputProps={{
                            ...
                            inputProps={{ 
                                min: 0, 
                                style: { textAlign: 'center', fontWeight: 'bold', color, fontSize: '16px' } 
                            }}
                            sx={{ 
                                width: '60px',
...
// Sub-componente de Antecedente
const AntecedenteAccordion = ({ character, editMode, updateField }) => (
...
// Sub-componente de Descrição
const DescricaoAccordion = ({ character, editMode, updateField }) => (

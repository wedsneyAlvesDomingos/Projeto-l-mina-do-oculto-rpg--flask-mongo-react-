...
const DescansoPanel = React.memo(({ open, onClose, onDescansoCurto, onDescansoLongo }) => (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ backgroundColor: 'var(--bg-dark)', color: 'white', display: 'flex', alignItems: 'center', gap: 1 }}>
            <HotelIcon /> Sistema de Descanso
        </DialogTitle>
        <DialogContent sx={{ pt: 3 }}>
            <Typography variant="body2" sx={{ mb: 3, color: 'var(--text-primary)' }}>
                Escolha o tipo de descanso para recuperar seus recursos.
            </Typography>
            <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 2, backgroundColor: 'var(--surface-default)', height: '100%' }}>
                        <Typography variant="h6" sx={{ color: 'var(--text-primary)', fontWeight: 'bold', mb: 1 }}>Descanso Curto</Typography>
                        <Typography variant="body2" sx={{ mb: 2, color: 'var(--text-primary)' }}>
                            Duração: 1 hora<br/>Intervalo: a cada 6 horas
                        </Typography>
                        <Typography variant="caption" sx={{ display: 'block', mb: 2, color: 'var(--text-primary)' }}>
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 2, backgroundColor: 'var(--surface-default)', height: '100%' }}>
                        ...
                        <Button variant="contained" fullWidth onClick={onDescansoLongo} sx={{ backgroundColor: 'var(--bg-dark)' }}>
                            Descansar (Longo)
                        </Button>
                    </Paper>
                </Grid>

...
class CatalogEspecie(Base):
    ...
    subespecies = relationship(
        "CatalogSubespecie", back_populates="especie", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        return {
            ...
            "nome": self.display_name,
            "pvBase": self.pv_base,
            "velocidadeBase": float(self.velocidade_base),
...
class CatalogSubespecie(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    especie_id = Column(
        Integer, ForeignKey("catalog_especies.id", ondelete="CASCADE"), nullable=False
    )
...
class CatalogItem(Base):
    ...
    def to_dict(self):
        return {
            ...
            "categoria": self.categoria,
            "descricao": self.descricao,
            "pesoKg": float(self.peso_kg or 0),
            "precoMO": self.preco_mo,
            "dano": self.dano,
            "tipoDano": self.tipo_dano,
            "criticoMargem": self.critico_margem,
            "defesa": self.defesa,
            "bonusDefesa": self.bonus_defesa,
...
class CatalogAbility(Base):
    ...
    def to_dict(self):
        return {
            ...
            "tipo": self.tipo,
            "descricao": self.descricao,
            "custoAcoes": self.custo_acoes,
            "custoMagia": self.custo_magia,
            "custoEstamina": self.custo_estamina,
            "alcance": float(self.alcance_m) if self.alcance_m else None,
            "duracao": self.duracao_texto,
            "cooldown": self.cooldown_tipo,
            "efeito": self.efeito or {},
            "efeitoOpcional": self.efeito_opcional or {},
            "escalamento": self.escalamento or {},
...
class CatalogClasse(Base):
    ...
    def to_dict(self):
        return {
            ...
            "nome": self.display_name,
            "tipo": self.tipo,
            "aprendizSlug": self.aprendiz_slug,
            ...
            "habilidadeClasse": (
                ...
                if self.habilidade_classe_nome
                else None
            ),
        }
...
class CatalogClasseEspecializacao(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    classe_id = Column(
        Integer, ForeignKey("catalog_classes.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint("classe_id", "especializacao_id", name="uq_classe_espec"),
    )
...
class CatalogArvoreRegalia(Base):
    ...
    niveis = relationship(
        "CatalogArvoreNivel", back_populates="arvore", cascade="all, delete-orphan"
    )
...
class CatalogArvoreNivel(Base):
    ...
    arvore_id = Column(
        Integer,
        ForeignKey("catalog_arvores_regalia.id", ondelete="CASCADE"),
        nullable=False,
    )
    nivel = Column(Integer, nullable=False, default=1)
    regalia_id = Column(
        Integer, ForeignKey("catalog_regalias.id", ondelete="CASCADE"), nullable=False
    )
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "arvore_id": self.arvore_id,
            "nivel": self.nivel,
            "regalia_id": self.regalia_id,
        }
...
class CatalogRegalia(Base):
    ...
    profissao_id = Column(
        Integer, ForeignKey("catalog_profissoes.id", ondelete="SET NULL")
    )
    bonus_habilidades = Column(JSONB, default=dict)
    ...
    abilities = relationship(
        "CatalogRegaliaAbility", back_populates="regalia", cascade="all, delete-orphan"
    )
...
class CatalogRegaliaAbility(Base):
    ...
    ability_id = Column(
        Integer, ForeignKey("catalog_abilities.id", ondelete="CASCADE"), nullable=False
    )
    condicional = Column(String)
    __table_args__ = (
        UniqueConstraint("regalia_id", "ability_id", name="uq_regalia_ability"),
    )
...
class CatalogProfissao(Base):
    ...
    habilidades = relationship(
        "CatalogProfissaoHabilidade",
        back_populates="profissao",
    ...
    def to_dict(self):
        return {
            ...
            "nome": self.display_name,
            "descricao": self.descricao,
            "rendaPorDia": self.renda_por_dia_mo,
...
class CatalogProfissaoHabilidade(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    profissao_id = Column(
        Integer, ForeignKey("catalog_profissoes.id", ondelete="CASCADE"), nullable=False
    )
    ...
    ability_id = Column(
        Integer, ForeignKey("catalog_abilities.id", ondelete="SET NULL")
    )
    rule_version = Column(String, nullable=False, default="0.5")
...
class User(Base):
    ...
    personagens = relationship(
        "Personagem", back_populates="user", cascade="all, delete-orphan"
    )
...
class Personagem(Base):
    ...
    especie_id = Column(
        Integer, ForeignKey("catalog_especies.id", ondelete="SET NULL"), nullable=True
    )
    subespecie_id = Column(
        Integer,
        ForeignKey("catalog_subespecies.id", ondelete="SET NULL"),
        nullable=True,
    )
    antecedente_id = Column(
        Integer,
        ForeignKey("catalog_antecedentes.id", ondelete="SET NULL"),
        nullable=True,
    )
    classe_aprendiz_id = Column(
        Integer, ForeignKey("catalog_classes.id", ondelete="SET NULL"), nullable=True
    )
    classe_primaria_id = Column(
        Integer, ForeignKey("catalog_classes.id", ondelete="SET NULL"), nullable=True
    )
    especializacao_id = Column(
        Integer, ForeignKey("catalog_classes.id", ondelete="SET NULL"), nullable=True
    ...
    regalia_points_total = Column(Integer, nullable=False, server_default="0")
    regalia_points_spent = Column(
    ...
    sections = relationship(
        "CharacterSection", back_populates="character", cascade="all, delete-orphan"
    )
    ...
    proficiencies = relationship(
        "CharacterProficiency", back_populates="character", cascade="all, delete-orphan"
    )
    regalias = relationship(
        "CharacterRegalia", back_populates="character", cascade="all, delete-orphan"
    )
    effects = relationship(
        "CharacterEffect", back_populates="character", cascade="all, delete-orphan"
    )
    audit_logs = relationship(
        "CharacterAuditLog", back_populates="character", cascade="all, delete-orphan"
    )
    # v2 relationships
    char_skills_v2 = relationship(
        "CharacterSkillV2", back_populates="character", cascade="all, delete-orphan"
    )
    char_proficiencias = relationship(
        "CharacterProficienciaV2",
        back_populates="character",
        cascade="all, delete-orphan",
    )
    char_regalias_v2 = relationship(
        "CharacterRegaliaV2", back_populates="character", cascade="all, delete-orphan"
    )
    char_abilities = relationship(
        "CharacterAbilityV2", back_populates="character", cascade="all, delete-orphan"
    )
    char_effects_v2 = relationship(
        "CharacterEffectV2", back_populates="character", cascade="all, delete-orphan"
    )
    level_snapshots = relationship(
...
class AttributeDefinition(Base):
    ...
    attributes = relationship(
        "CharacterAttribute", back_populates="definition", cascade="all, delete-orphan"
    )
...
class CharacterAttribute(Base):
    ...
    attribute_id = Column(
        Integer, ForeignKey("attribute_definitions.id"), nullable=False
    )
...
class Item(Base):
    ...
    inventories = relationship(
        "CharacterInventory", back_populates="item", cascade="all, delete-orphan"
    )
...
class Condition(Base):
    ...
    characters = relationship(
        "CharacterCondition", back_populates="condition", cascade="all, delete-orphan"
    )
...
class Regalia(Base):
    ...
    characters = relationship(
        "CharacterRegalia", back_populates="regalia", cascade="all, delete-orphan"
    )
...
class CharacterSection(Base):
    ...
    __table_args__ = (
        UniqueConstraint("character_id", "section_key", name="uq_character_section"),
    )
...
class CharacterEffect(Base):
    ...
    )  # 'pocao', 'veneno', 'regalia', 'condicao', 'kit', 'material', 'outro'
    source = Column(String, nullable=True)  # nome/id da origem
    data = Column(JSONB, default={})  # dados mecânicos livres
    remaining_rounds = Column(
        Integer, nullable=True
    )  # rodadas restantes (NULL = sem duração)
    remaining_uses = Column(
        Integer, nullable=True
    )  # usos restantes (NULL = sem limite)
...
class CharacterAuditLog(Base):
    ...
    action = Column(
        Text, nullable=False
    )  # 'create', 'update', 'delete', 'rest', 'condition_add', etc.
    before_state = Column(JSONB, nullable=True)
    after_state = Column(JSONB, nullable=True)
    metadata_ = Column(
        "metadata", JSONB, default={}
    )  # 'metadata' é nome reservado em SQLAlchemy
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class CharacterSkillV2(Base):
    ...
    __table_args__ = (
        UniqueConstraint("character_id", "skill_id", name="uq_char_skill_v2"),
    )
    ...
    skill_id = Column(
        Integer, ForeignKey("catalog_skills.id", ondelete="CASCADE"), nullable=False
    )
...
class CharacterProficienciaV2(Base):
    ...
    __table_args__ = (
        UniqueConstraint("character_id", "proficiencia_id", name="uq_char_prof_v2"),
    )
    ...
    proficiencia_id = Column(
        Integer,
        ForeignKey("catalog_proficiencias.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class CharacterRegaliaV2(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "character_id", "regalia_id", "opcao_escolhida", name="uq_char_regalia_v2"
        ),
    )
...
class CharacterAbilityV2(Base):
    ...
    __table_args__ = (
        UniqueConstraint("character_id", "ability_id", name="uq_char_ability_v2"),
    )
...
class CharacterEffectV2(Base):
    ...
    condicao_id = Column(
        Integer, ForeignKey("catalog_condicoes.id", ondelete="SET NULL")
    )
    ...
    def to_dict(self):
        ...
        return {
            "id": self.id,
            "condicao_id": self.condicao_id,
            "slug": self.effect_slug or (self.condicao.slug if self.condicao else None),
            "nome": self.display_name
            or (self.condicao.display_name if self.condicao else None),
            "nivel": self.nivel,
            "fonteTipo": self.fonte_tipo,
            "fonteSlug": self.fonte_slug,
            ...
            "duracaoTipo": self.duracao_tipo,
            "duracaoValor": self.duracao_valor,
